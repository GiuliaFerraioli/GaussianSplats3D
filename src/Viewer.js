...
export class Viewer {
    ...
    constructor(options = {}) {
        ...
        this.controllers = [];
        this.controllerGripModels = [];
        this.clock = new THREE.Clock(); // For delta timing in VR movement
        this.vrSpeed = 2.0; // units per second
    ...
    setupWebXR(webXRSessionInit) {
        if (this.webXRMode) {
            ...
            this.renderer.xr.addEventListener('sessionstart', () => { this.webXRActive = true; });
            this.renderer.xr.addEventListener('sessionend', () => { this.webXRActive = false; });
            ...
            // Add controllers (without models)
            for (let i = 0; i < 2; i++) {
                const controller = this.renderer.xr.getController(i);
                this.threeScene.add(controller);
    ...
    selfDrivenUpdate() {
        ...
        if (this.webXRActive) {
            this.updateVRControllers(delta);
            // no need to call setAnimationLoop here — it's already running
        }
